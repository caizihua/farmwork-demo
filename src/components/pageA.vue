    ...
    <v-card elevation="0" class="d-flex">
      ...
      <div class="ma-3" :style="{width:`${wrapWidth-250}px`}">
        <div class="d-flex align-center mb-3">
          <span class="mr-2" style="width:3px; height:25px; background-color: #5D7CC5;"></span>
          <div>
            <v-menu offset-x v-model="firstObj.menuDialog">
              <template v-slot:activator="{ on, attrs }">
                <div class="pr-2" v-bind="attrs" v-on="on">{{firstObj.title}}</div>
              </template>
              <v-card class="d-flex flex-column" elevation="0">
                <v-menu offset-x
                        v-model="firstObj.entryDialog"
                        :close-on-content-click="false">
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn v-bind="attrs" v-on="on" color="primary" text @click="addEntry('firstObj')">选择子标题</v-btn>
                  </template>
                  <v-card class="d-flex flex-column pa-2" style="max-width:210px">
                    <v-text-field
                ...
                <v-btn color="primary" text @click="clearLEntry('firstObj')">清空子标题</v-btn>
              </v-card>
            </v-menu>
          </div>
        </div>
...
export default {
  ...
  created () {
    ...
    this.templateData.dataSource.data.forEach(e => {
      if (e.isCreate === false) {
